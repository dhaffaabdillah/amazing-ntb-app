...
import 'package:travel_hour/utils/list_card_report.dart';
import 'package:travel_hour/utils/loading_cards.dart';
import 'package:travel_hour/utils/snacbar.dart';
import 'package:easy_localization/easy_localization.dart';

class SearchReportPage extends StatefulWidget {
  SearchReportPage({Key? key}) : super(key: key);

  _SearchReportPageState createState() => _SearchReportPageState();
}
...
class _SearchReportPageState extends State<SearchReportPage> {
  ...
  @override
  void initState() {
    Future.delayed(Duration()).then((value) => context.read<SearchReportBloc>().saerchInitialize());
    super.initState();
  }
      ...
      body: Column(
          ...
          children: <Widget>[
            ...
            Padding(
              ...
              child: Text(
                context.watch<SearchReportBloc>().searchStarted == false
                    ? 'recent searchs'
                    : 'we have found',
      ...
      child: TextFormField(
        ...
        decoration: InputDecoration(
          ...
          suffixIcon: IconButton(
            ...
            onPressed: () {
              context.read<SearchReportBloc>().saerchInitialize();
            },
          ),
        ),
        ...
        onFieldSubmitted: (value) {
          if (value == '') {
            ...
            context.read<SearchReportBloc>().setSearchText(value);
            context.read<SearchReportBloc>().addToSearchList(value);
          }
        },
      ),
    );
...
class SuggestionsUI extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final sb = context.watch<SearchReportBloc>();
    return Expanded(
      child: sb.recentSearchData.isEmpty
          ? EmptyPage(
              icon: Feather.search,
              message: 'search for reports'.tr(),
...
class AfterSearchUI extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Expanded(
      child: FutureBuilder(
        future: context.watch<SearchReportBloc>().getData(),
